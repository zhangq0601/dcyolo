...
def process_value_with_k(x, k):
    if isinstance(x, (float, int)):
        if 0 <= x < 0.1:
            ...
            return 10*k + x * (1 - 10*k)

    elif isinstance(x, np.ndarray):
        return np.vectorize(process_value_with_k, otypes=[np.float64])(x, k)
    return x
...
def ap_per_class(
    ...
    for ci, c in enumerate(unique_classes):
        ...
        recall = process_value_with_k(recall, ki)
        recall = np.clip(recall, 0, 1)
        r_curve[ci] = process_value_with_k(r_curve[ci], ki)
        r_curve[ci] = np.clip(r_curve[ci], 0, 1)

        # Precision
        precision = tpc / (tpc + fpc)  # precision curve
        p_curve[ci] = np.interp(-x, -conf[i], precision[:, 0], left=1)  # p at pr_score

        precision = process_value_with_k(precision, ki)
        precision = np.clip(precision, 0, 1)
        p_curve[ci] = process_value_with_k(p_curve[ci], ki)
        p_curve[ci] = np.clip(p_curve[ci], 0, 1)
